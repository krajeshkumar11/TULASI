...
# Updated balance each month = (Monthly unpaid balance) + (Monthly interest ratex
...
def var_payingdebtoffinayear(var_balance, var_annualinterestrate):
    ...
    while True:
        ...
        while i != 0:
            ...
            var_bal = var_ubal + ((var_annualinterestrate / 12.0) * var_ubal)
            i -= 1
        if var_bal > 0:
            var_pay += 10
        else:
...
def main():
    ...
    print("Lowest Payment: " + str(var_payingdebtoffinayear(data[0], data[1])))
